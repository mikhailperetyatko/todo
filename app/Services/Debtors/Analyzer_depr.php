...
class Analyzer
{
    ...
    protected function calculateAmountAccruals(array $servies, Carbon $onDate = null) : float
    {
        $balance = 0;
        ...
        foreach ($servies as $service) {
            $balance += round(($service['tariff'] * $service['amount'] * $koef), 2);
        }
    ...
    public function getBalance() : float
    {
        ...
        foreach($this->account->periods as $period) {
            ...
            $balance += $this->calculateAmountAccruals($period['services']);
    ...
    public function getDebtPeriods() : array
    {
        ...
        foreach(array_reverse($this->account->periods) as $period) {
            $accrual = $this->calculateAmountAccruals($period['services']);
            $periods[] = [
                'period' => $period,
    ...
    {
        $calculations = [];
        $periods = $this->getDebtPeriods();
        $dates = $this->getDates($periods);
               
        dd($periods, $dates);
        $start = array_shift($periods);
        
        foreach ($ownerDates as $key => $date) {
            if ($key < (count($dates) - 1)) $date->subDay();
