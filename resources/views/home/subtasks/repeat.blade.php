@if($subtask->task->subtasks()->where('finished', 0)->count() == 1)
    <div class="card">
        <div class="card-body">
            ...
            @if(! $subtask->task->repeatability)
                <div class="form-group">
                    <label for="taskDate">Дата выполнения задачи</label>
                    <input type="date" class="form-control {{ $errors->get('date') ? 'is-invalid' : '' }}" name="date" id="taskDate" placeholder="Дата" value="{{ old('date') }}">
                ...
                <div class="form-group">
                    ...
                    <input type="time" class="form-control {{ $errors->get('time') ? 'is-invalid' : '' }}" id="taskTime" name="time" placeholder="Время" value="{{ old('time') }}">
                    <div class="invalid-feedback">
